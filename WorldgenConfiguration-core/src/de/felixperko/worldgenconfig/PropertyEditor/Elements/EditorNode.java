...
public class EditorNode extends DisplayNode {
	
	static HashMap<Class<? extends Node>, Integer> count = new HashMap<>();
	...
	Connector[] inputs = new Connector[2];
	...
	public EditorNode(EditorStage stage, float x, float y, Class<? extends Node> node2) {
		this.stage = stage;
		try {
			this.node = node2.newInstance();
		} catch (InstantiationException | IllegalAccessException e) {
			e.printStackTrace();
		}
		inputs = new Connector[this.node.getInputCount()];
	...
	private int incrementCount() {
		Integer val = count.get(node.getClass());
		if (val == null)
			val = 1;
		else
			val = val+1;
		count.put(node.getClass(), val);
		return val;
	}

	@Override
	public void init() {
		label = new VisLabel(node.getDisplayName()+"\n"+text);
	...
	public void drawImage() {
		...
		pm.setColor(new Color(0.1f, 0.1f, 0.1f, 1));
	...
	protected void doubleclicked() {
		stage.addActor(new NodeSettingWindow(this));
	}
